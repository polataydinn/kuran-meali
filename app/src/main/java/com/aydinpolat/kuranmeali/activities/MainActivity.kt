...
class MainActivity : AppCompatActivity() {
    ...
    private val listOfSuras = mutableListOf<Suras>()
    ...
    private fun setAyetsToRoom() {
        ...
        stringBuilder.lines().forEach { sura ->
            if (sura == ",") {
                listOfSuras.add(
                    suraCounter,
                    Suras(Constants.suraNames[suraCounter], suraCounter, "", listOfAyats)
                )
                suraCounter++
                listOfAyats = mutableListOf<Ayats>()
                ayatsCounter = 0
            } else {
                ...
                if (sura.startsWith("a") && sura.contains("Bkz")) {
                    ...
                    val ayatsNumber = sura.substringBefore(".").replace("#", "-").removePrefix("a")
                    listOfAyats.add(
                        ayatsCounter,
                        Ayats(
                            sura.substringAfter("."),
